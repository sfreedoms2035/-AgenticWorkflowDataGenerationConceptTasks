...
def extract_semantic_blocks(text):
    ...
    for match in matches:
        block_name = match.group(1).strip().replace('\\', '')
...
def unescape_markdown(text):
    """Remove Markdown backslash escapes that Gemini's DOM rendering adds.
    
    When scraping Gemini's response from the DOM, characters like _, !, <, >, [, ], (, )
    are often backslash-escaped (e.g., training\_data\_id becomes training_data_id).
    This must run BEFORE any JSON parsing or block extraction.
    """
    if not text:
        return text
    # Unescape markdown-special characters (NOT double-quote — that's JSON escaping):
    # \_  \!  \<  \>  \[  \]  \(  \)  \#  \*  \&  \.  \-  \`  \~  \|
...
def heuristic_extract_blocks(text):
    ...
    mappings = {
        ...
        'CONCEPT_HEADER': [r'CONCEPT_HEADER', r'Concept Header'],
        'ONTOLOGICAL_SCAFFOLDING': [r'ONTOLOGICAL_SCAFFOLDING', r'Ontological Scaffolding'],
        'ABSTRACTION_LEVEL': [r'ABSTRACTION_LEVEL', r'Abstraction Level'],
        'AXIOMATIC_BASE': [r'AXIOMATIC_BASE', r'Axiomatic Base'],
        'RELATIONAL_NETWORK': [r'RELATIONAL_NETWORK', r'Relational Network'],
        'INFERENTIAL_FRAMEWORK': [r'INFERENTIAL_FRAMEWORK', r'Inferential Framework'],
        'METHODOLOGICAL_APPARATUS': [r'METHODOLOGICAL_APPARATUS', r'Methodological Apparatus'],
        'ILLUSTRATIVE_CORPUS': [r'ILLUSTRATIVE_CORPUS', r'Illustrative Corpus'],
        'GOAL_ORIENTATION': [r'GOAL_ORIENTATION', r'Goal Orientation', r'Goal'],
        'LIMITATIONS_AND_RISKS': [r'LIMITATIONS_AND_RISKS', r'Limitations', r'Risks'],
        'INTER_CONCEPT_RELATIONSHIPS': [r'INTER_CONCEPT_RELATIONSHIPS', r'Inter-Concept Relationships'],
    }
...
def validate_and_save_json(llm_response, out_json_path, thinking_text=None):
    ...
    try:
        ...
        # 0b. Clean repetition loops first
        ...
        # Priority chain for CoT extraction:
        #   A) Raw <think>...</think> block in the response (Gemini often outputs this BEFORE the !!!!! delimiters)
        #   B) !!!!!REASONING!!!!! semantic block
        #   C) thinking_text from Playwright UI scraping
        
        reasoning_main = ""
        
        # A) Extract raw <think>...</think> from the full response (often appears before !!!!!METADATA!!!!!)
        think_match = re.search(r'<think>(.*?)</think>', llm_response, re.DOTALL | re.IGNORECASE)
        if not think_match:
            # Try escaped variants: \<think\>...\</think\> or \\<think\\>...\\</think\\>
            think_match = re.search(r'\\?<think\\?>(.*?)\\?</think\\?>', llm_response, re.DOTALL | re.IGNORECASE)
        
        if think_match and len(think_match.group(1).strip()) > 500:
            reasoning_main = think_match.group(1).strip()
            log(f"  [Reasoning] Extracted raw <think> block: {len(reasoning_main)} chars")
        
        # B) Try !!!!!REASONING!!!!! semantic block
        if not reasoning_main or len(reasoning_main) < 500:
            reasoning_block = clean_semantic_block(blocks.get("REASONING", ""))
            if reasoning_block and len(reasoning_block) > len(reasoning_main):
                reasoning_main = reasoning_block
                log(f"  [Reasoning] Using !!!!!REASONING!!!!! block: {len(reasoning_main)} chars")
        
        # C) Fallback to Playwright UI thinking_text
        if (not reasoning_main or len(reasoning_main) < 500) and thinking_text and len(thinking_text) > 500:
            reasoning_main = thinking_text
            log(f"  [Reasoning] Using Playwright UI thinking_text: {len(reasoning_main)} chars")
        
        if not reasoning_main:
            reasoning_main = "(Missing REASONING — no <think> block, no !!!!!REASONING!!!!! block, no UI thinking_text)"
            log("  ⚠️ [Reasoning] No reasoning content found from any source!")
        ...
        # Try Granular
        # Merge results into final object for Concept Tasks
        assistant_content_obj = {
            "concept_header": clean_semantic_block(blocks.get("CONCEPT_HEADER", mono_obj.get("concept_header", ""))),
            "ontological_scaffolding": clean_semantic_block(blocks.get("ONTOLOGICAL_SCAFFOLDING", mono_obj.get("ontological_scaffolding", ""))),
            "abstraction_level": clean_semantic_block(blocks.get("ABSTRACTION_LEVEL", mono_obj.get("abstraction_level", ""))),
            "axiomatic_base": clean_semantic_block(blocks.get("AXIOMATIC_BASE", mono_obj.get("axiomatic_base", ""))),
            "relational_network": clean_semantic_block(blocks.get("RELATIONAL_NETWORK", mono_obj.get("relational_network", ""))),
            "inferential_framework": clean_semantic_block(blocks.get("INFERENTIAL_FRAMEWORK", mono_obj.get("inferential_framework", ""))),
            "methodological_apparatus": clean_semantic_block(blocks.get("METHODOLOGICAL_APPARATUS", mono_obj.get("methodological_apparatus", ""))),
            "illustrative_corpus": clean_semantic_block(blocks.get("ILLUSTRATIVE_CORPUS", mono_obj.get("illustrative_corpus", ""))),
            "goal_orientation": clean_semantic_block(blocks.get("GOAL_ORIENTATION", mono_obj.get("goal_orientation", ""))),
            "limitations_and_risks": clean_semantic_block(blocks.get("LIMITATIONS_AND_RISKS", mono_obj.get("limitations_and_risks", ""))),
            "inter_concept_relationships": clean_semantic_block(blocks.get("INTER_CONCEPT_RELATIONSHIPS", mono_obj.get("inter_concept_relationships", "")))
        }
        
        # If mono_obj was fully intact and not all empty, just use it
        if mono_obj and any(mono_obj.values()):
            # Override any empty heuristic blocks with original mono_obj
            for k in assistant_content_obj.keys():
                if not assistant_content_obj[k] and k in mono_obj:
                    assistant_content_obj[k] = mono_obj[k]
...
DO NOT use markdown fenced code blocks (like ```json or ```) ANYWHERE in your response. Fenced blocks trigger Canvas mode and crash the agent. Write all code and JSON as raw plain-text indented by 4 spaces.
...
"""
    ...
    with open(out_prompt, 'w', encoding='utf-8') as f:
        f.write(mega_prompt)
    log(f"Prompt saved to Output/prompts/{core_name}_prompt.txt")
