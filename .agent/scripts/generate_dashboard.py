...
def generate_dashboard():
    ...
    corpus_arr = [d.get("corpus_lines", 0) for d in task_results.values() if d.get("corpus_lines")]
    sysml_arr = [d.get("sysml_elements", 0) for d in task_results.values() if d.get("sysml_elements")]
    ...
    stats_html += stats_card("Illustrative Corpus Lines", corpus_arr)
    stats_html += stats_card("SysML Elements", sysml_arr)
    ...
    for tk in sorted(task_results.keys()):
        ...
        corpus = f'{d.get("corpus_lines", 0)}'
        sysml = f'{d.get("sysml_elements", 0)}'
        ...
        task_rows += f"""<tr class="{row_class}">
            <td>{icon} {tk}</td>
            <td>{cot}</td><td>{ans}</td><td>{corpus}</td><td>{sysml}</td>
    ...
    <div class="task-table-wrapper">
        <table>
            <thead>
                <tr>
                    <th>Task</th>
                    <th>CoT</th><th>Answer</th><th>Corpus</th><th>SysML</th>
