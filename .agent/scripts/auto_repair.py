...
def repair_metadata(task, filename):
    ...
    for key in REQUIRED_KEYS:
        if key not in task or not task[key] or task[key] == "..." or task[key] == "{pdf_name}":
            if key == "training_data_id": task[key] = f"TD-CON-{context['doc']}-T{context['turn']}t{context['task']}-{today}-v1.0"
            elif key == "prompt_version": task[key] = "ConceptTasks_v1.5"
            ...
            elif key == "task_type": task[key] = "concept_task"
            elif key == "affected_role": task[key] = "Senior Software Engineer / Automotive Architect"
            elif key == "date_of_generation": task[key] = today
            elif key == "key_words": task[key] = ["AD/ADAS", "Conceptual Framework", "Systems Engineering"]
            elif key == "summary": task[key] = f"Complex ADAS conceptual task for {context['doc']} - Turn {context['turn']}"
...
def repair_missing_cot_numbers(task):
    ...
    for conv in task.get("conversations", []):
        if conv.get("role") == "assistant":
            ...
            # ── Phase 1: De-stutter pass ──────────────────────────────
            # Fix tripled/doubled numbering like "1. 1. 1. Title" → "1. Title"
            # and "1.2. 1.2. 1.2. Title" → "1.2. Title"
            # Pattern: a prefix like "N." or "N.M." repeated 2+ times
            reasoning = re.sub(
                r'(\d+(?:\.\d+)?\.)\s+(?:\1\s+)+',
                r'\1 ',
                reasoning
            ...
            # ── Phase 2: Prefix injection (only if truly missing) ─────
            for title, prefix in COT_TITLES_TO_NUMBERS.items():
                ...
                # Check if the title already has ANY numeric prefix before it.
                # Look for patterns like "1. Title", "1.2. Title", "**1.** Title", etc.
                already_has_prefix = re.search(
                    rf'(?:\d+\.(?:\d+\.)?)\s*(?:\*\*\s*)?{escape_title}',
                    reasoning
                )
                
                if already_has_prefix:
                    # Already has a numeric prefix — skip to avoid stuttering
                    continue
                
                # Title exists without a prefix — inject one
                # Only match when preceded by newline, start-of-string, or ** formatting
                pattern = rf'((?:^|\\\\n|\\n|\n)\s*(?:\*\*\s*)?)({escape_title})'
                
                def repl(m, _prefix=prefix):
...
def repair_concept_header(task):
    """Synthesize a concept_header block from existing metadata if missing."""
    convs = task.get("conversations", [])
    if len(convs) < 2:
        return False
    ...
    # Skip if already has a populated concept_header
    existing = parsed.get("concept_header", "")
    if existing and len(existing.strip()) >= 50:
        return False
    
    # Synthesize from metadata
    tid = task.get("training_data_id", "UNKNOWN")
    concept_id = tid.replace("TD-CON-", "CON-").replace("TD-CODING-", "CON-") if tid else "CON-UNKNOWN"
    summary = task.get("summary", "Unnamed Concept")
    keywords = task.get("key_words", [])
    role = task.get("affected_role", "Senior Engineer")
    doc = task.get("document", "Unknown Document")
    gen_date = task.get("date_of_generation", "2026-01-01")
    
    header = (
        f"- **Concept ID:** {concept_id}\n"
        f"- **Canonical Name:** {summary[:120]}\n"
        f"- **Definition Version:** 1.0\n"
        f"- **Last Definition Update Date:** {gen_date}\n"
        f"- **Estimated Origin Date/Period:** Derived from {doc}\n"
        f"- **Primary Domain(s) of Application:** Autonomous Driving Systems, {role.split()[-1] if role else 'Engineering'}\n"
        f"- **Assigned Abstraction Level:** 2 (Framework/Paradigm Concept)\n"
        f"- **Keywords:** {', '.join(keywords[:8]) if keywords else 'AD, ADAS, Safety'}\n"
        f"- **Source Reference(s):** {doc}"
    )
    
    parsed["concept_header"] = header
    convs[1]["content"] = json.dumps(parsed, indent=2, ensure_ascii=False)
    return True
...
def auto_repair(filepath):
    ...
    if repair_concept_header(task): repair_log["fixes_applied"].append("concept_header_synthesized")
